...
'			Tmp = Split(G_ID,",")
'			For i=0 To Ubound(Tmp)
'				Call BBS_Page_Html(Trim(Tmp(i)))
'				
'				Sql = "select pID from Ok3w_Guest where ID=" & Tmp(i)
'				pID = Conn.Execute(Sql)(0)
'				If pID<>0 Then
'					Call BBS_Page_Html(pID)
'				End If
'			Next
'			
'			Call BBS_List_Html(1)
'		End If
	End Sub
    ...
    Private Sub GetFormData()
		...
		If InStr("|" & Lcase(Application("Ok3w_SiteUserRegKillName")) & "|","|" & Lcase(UserName) & "|")>0 Then
			Call MessageBox("禁止使用该用户发言，请重新输入...","")
		End If
