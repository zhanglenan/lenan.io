...
Class Ok3w_Article
    ...
    Private Sub Class_Initialize()
		...
		Hits = 0
		AddTime = Now()
		Inputer = Request.Cookies("Ok3w")("AdminName")
		ComeFrom = Application("Ok3w_SiteName")
		Author = Application("Ok3w_SiteName")
    End Sub
    '类终止
	...
	Public Sub User_Give_Jifen(ID)
		Sql = "update Ok3w_User set Jifen=Jifen+" & Application("Ok3w_SiteUserTGJifen") & " where User_Name in(select Inputer from Ok3w_Article where ID in(" & ID & ") and IsUserAdd=1 and GiveJifen=0)"
		Conn.Execute Sql
		
		Sql = "update Ok3w_Article set GiveJifen=" & Application("Ok3w_SiteUserTGJifen") & " where ID in(" & ID & ") and IsUserAdd=1 and GiveJifen=0"
		Conn.Execute Sql	
	End Sub
	...
	Public Sub User_Article_Save(a_ID)
        ...
        ComeFrom = Request.Form("ComeFrom")
        AddTime = Now()
		Inputer = Request.Cookies("Ok3w")("User_Name")
		IsPass = 0
		IsPic = 0
        PicFile = ""
        IsTop = 0
