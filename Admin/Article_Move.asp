...
If action = "move" Then
	...
	If strClassID="" Then
		...
		Call ActionOk("Article_Move.asp?ChannelID=" & ChannelID)
	Else
		...
		Call ActionOk("Article_List.asp?ChannelID=" & ChannelID)
	End If
End If
%>
<html>
          ...
          valign="top"><div id="tabContent__0" style="DISPLAY: block; VISIBILITY: visible">
            ...
            bgcolor="#8ccebd" border="0">
                ...
                valign="top" bgcolor="#fffcf7"><table border="0" cellpadding="5" cellspacing="1" bgcolor="#EBEBEB">
                      <form id="Form" name="Form" method="post" action="?action=move&ChannelID=<%=ChannelID%>&strClassID=<%=strClassID%>">
                        <tr bgcolor="#FFFFFF">
                          <td><%If strClassID<>"" Then%>文章ID：<br>
                            <textarea name="textarea" rows="20" disabled="disabled"><%=strClassID%></textarea>
							<%Else%>
                            原类别：<br>
                            <select name="formClassID" size="20" id="formClassID">
                            <%Call InitClassSelectOption(ChannelId,0,0)%>
                          </select><%End If%></td>
                          <td>转移到：<br>
                            <select name="toClassID" size="20" id="toClassID">
                            <%Call InitClassSelectOption(ChannelId,0,0)%>
                          </select></td>
                          </tr>
