...
If Cmd <> "" Then
	...
	Select Case CmdTmp(0)
		Case "pass"
			...
			Set Soft = Nothing
			Call SaveAdminLog("开通/关闭软件，Action=" & CmdTmp(1) & "，ID=" & Lid)
			Call ActionOk("Soft_List.asp?ChannelID=" & ChannelID & "&IsDelete=" & IsDelete)
		Case "del"
			Call Soft.Del(Lid)
			Set Soft = Nothing
			Call SaveAdminLog("删除软件，ID=" & Lid)
			Call ActionOk("Soft_List.asp?ChannelID=" & ChannelID & "&IsDelete=" & IsDelete)
		Case "top"
			Call Soft.Top(CmdTmp(1),Lid)
			Set Soft = Nothing
			Call SaveAdminLog("置顶/取消软件，Action=" & CmdTmp(1) & "，ID=" & Lid)
			Call ActionOk("Soft_List.asp?ChannelID=" & ChannelID & "&IsDelete=" & IsDelete)
		Case "commend"
			Call Soft.Commend(CmdTmp(1),Lid)
			Set Soft = Nothing
			Call SaveAdminLog("推荐/取消软件，Action=" & CmdTmp(1) & "，ID=" & Lid)
			Call ActionOk("Soft_List.asp?ChannelID=" & ChannelID & "&IsDelete=" & IsDelete)
		Case "Pic"
			Call Soft.Pic(CmdTmp(1),Lid)
			Set Soft = Nothing
			Call SaveAdminLog("图片/取消软件，Action=" & CmdTmp(1) & "，ID=" & Lid)
			Call ActionOk("Soft_List.asp?ChannelID=" & ChannelID & "&IsDelete=" & IsDelete)
		Case "Okdel"
			Call Soft.OkDel(Lid)
			Set Soft = Nothing
			Call SaveAdminLog("彻底删除软件，ID=" & Lid)
			Call ActionOk("Soft_List.asp?ChannelID=" & ChannelID & "&IsDelete=" & IsDelete)
		Case "Hf"
            ...
            <td width="147"><table height="22" cellspacing="0" cellpadding="0" border="0">
              <tbody>
                <tr>
                  ...
                  <td 
                background="images/tab_active_bg.gif" class="tab"><strong class="mtitle">软件管理</strong></td>
                        ...
                        <form id="form1" name="form1" method="get" action="">
                          <tr>
                            <td colspan="8" align="left" bgcolor="#EBEBEB"><select name="stype" id="stype">
                              <option value="SoftName">按标题</option>
                              <option value="Softintro">按内容</option>
                              </select>
							  <select name="ClassID">
                            ...
                            <%Call InitClassSelectOption(ChannelId,0,ClassID)%>
                                ...
                                <a href="?ChannelID=<%=ChannelID%>">全部</a> <a href="?ChannelID=<%=ChannelID%>&IsPass=0">待审</a> <a href="?ChannelID=<%=ChannelID%>&IsTop=1">置顶</a> <a href="?ChannelID=<%=ChannelID%>&IsCommend=1">推荐</a> <a href="?ChannelID=<%=ChannelID%>&IsPic=1">图片</a> <a href="?ChannelID=<%=ChannelID%>&IsMove=1">滚动</a> <a href="?ChannelID=<%=ChannelID%>&IsPlay=1">轮播</a> </td>
                          </tr>
                        </form>
						<form id="form2" name="form2" method="post" action="?ChannelID=<%=ChannelID%>&IsDelete=<%=IsDelete%>">
                        <tr>
                          <td colspan="8" align="left" bgcolor="#EBEBEB"><%If IsDelete=0 Then%><input name="bntEdit" type="button" class="bntStyle" id="bntEdit" onClick="a_edit(this.form)" value="修改/查看" />
                                 <input name="bntCopy" type="button" class="bntStyle" id="bntCopy" onClick="a_copy(this.form)" value="复 制" />
                            ...
                            <input name="bntNotIsCommend" type="button" class="bntStyle" id="bntNotIsCommend" onClick="a_action(this.form,'commend|0')" value="取消推荐" /><%End If%><%If IsDelete=1 Then%>
	...
	%>
                          <tr>
                            ...
                            <td align="center" bgcolor="#FFFFFF">
                              ...
                              <%If Rs("Softimageurl")<>"" Then%>图片 <%End If%> 
